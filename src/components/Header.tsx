"use client";

import { useCallback, useEffect, useState } from "react";

function LambdaLogo() {
  return (
    <svg
      width="171"
      height="40"
      viewBox="0 0 171 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
    >
      <path
        d="M54.5472 27.5003H64.1365V32.1322H49.0749V7.745H54.5472V27.5003ZM1.46484 1.70898H38.1092V38.2959H1.46484V1.70898ZM4.70436 35.0712H34.8607V4.93062H4.70436V35.0712ZM11.1443 7.79363L17.5182 19.0937L10.4261 32.1808H14.9939L19.7209 23.2484L24.7545 32.1808H29.4064L15.7962 7.79059L11.1443 7.79363ZM104.823 14.0424C103.656 14.0154 102.504 14.3163 101.496 14.9116C100.612 15.4613 99.8454 16.1847 99.2422 17.0391H99.1701C98.4188 15.1943 96.6308 14.0363 94.377 14.0363C91.3718 14.0363 89.8332 15.6988 89.0819 16.8233H88.9738L88.7243 14.4345H84.2888V32.1261H89.5838V21.8199C89.5665 21.3524 89.6389 20.8858 89.7972 20.4462C90.1187 19.5344 90.9421 18.4554 92.3365 18.4554C94.0554 18.4554 94.8428 19.9751 94.8428 22.2181V32.1322H100.138V21.7986C100.135 21.3192 100.207 20.8423 100.351 20.3854C100.513 19.8386 100.843 19.3582 101.293 19.0135C101.743 18.6688 102.29 18.4776 102.854 18.4676C104.609 18.4676 105.433 19.9508 105.433 22.522V32.1444H110.725V21.7135C110.725 16.4313 108.185 14.0424 104.823 14.0424ZM145.879 7.05204H151.318V32.117H146.63L146.38 29.5123H146.308C145.199 31.5031 143.087 32.5152 140.833 32.5152C136.683 32.5152 133.356 28.9349 133.356 23.4338C133.32 17.4646 137.004 14.0272 141.191 14.0272C143.339 14.0272 145.019 14.787 145.806 16.0179H145.879V7.05204ZM145.879 21.9141C145.872 21.5878 145.848 21.2621 145.806 20.9385C145.485 19.4554 144.304 18.2244 142.624 18.2244C140.154 18.2244 138.868 20.4674 138.868 23.2545C138.868 26.2573 140.334 28.1386 142.588 28.1386C143.336 28.1471 144.063 27.8885 144.642 27.4081C145.22 26.9278 145.612 26.2566 145.749 25.5126C145.855 25.0989 145.903 24.6724 145.894 24.2453L145.879 21.9141ZM169.343 21.6588V32.117H164.655L164.333 30.3451H164.225C163.077 31.7553 161.28 32.5152 159.197 32.5152C155.642 32.5152 153.524 29.9105 153.524 27.087C153.524 22.4916 158.148 21.1422 163.828 21.1422V20.0814C163.828 19.1423 163.326 17.802 160.634 17.802C159.215 17.8218 157.815 18.1417 156.526 18.7411L154.783 15.5955C156.003 14.9086 158.407 14.0394 161.604 14.0394C167.455 14.0394 169.343 17.5133 169.343 21.674V21.6588ZM164.008 24.3942C161.003 24.3942 158.566 24.6009 158.554 26.8256C158.554 28.245 159.843 28.6279 161.063 28.6279C161.687 28.6326 162.295 28.4396 162.804 28.0759C163.313 27.7123 163.697 27.1963 163.9 26.6007C163.974 26.2933 164.01 25.9779 164.008 25.6616V24.3942ZM131.253 22.9475C131.253 29.3117 127.298 32.4908 123.343 32.4908C121.33 32.4908 119.389 31.731 118.153 29.637H118.102L117.892 32.0927H113.474V7.06724H118.836V16.5285H118.905C119.929 15.0089 121.729 14.0363 124.131 14.0363C128.263 14.0363 131.298 17.5406 131.265 22.9627L131.253 22.9475ZM125.777 23.1663C125.777 20.431 124.575 18.2488 122.072 18.2488C120.624 18.2488 119.283 19.3703 118.929 20.9598C118.858 21.2923 118.822 21.6317 118.824 21.9719V24.5765C118.821 24.9048 118.856 25.2323 118.929 25.5522C119.084 26.2815 119.48 26.9359 120.051 27.4085C120.622 27.881 121.334 28.1438 122.072 28.1538C124.38 28.169 125.789 26.3606 125.789 23.1815L125.777 23.1663ZM81.9027 21.6467V32.1048H77.2328L76.9112 30.3329H76.8031C75.6521 31.7432 73.858 32.503 71.7755 32.503C68.2174 32.5212 66.1139 29.9257 66.1139 27.1022C66.1139 22.5068 70.7387 21.1574 76.4184 21.1574V20.0814C76.4184 19.1423 75.9165 17.802 73.224 17.802C71.8058 17.8215 70.4074 18.1414 69.119 18.7411L67.373 15.5955C68.5961 14.9086 71.0002 14.0394 74.1976 14.0394C80.0366 14.0394 81.9238 17.5133 81.9238 21.674L81.9027 21.6467ZM76.5686 24.382C73.5635 24.382 71.1264 24.5887 71.1144 26.8135C71.1144 28.2328 72.4036 28.6158 73.6236 28.6158C74.2469 28.6209 74.8559 28.4281 75.365 28.0643C75.8739 27.7006 76.2571 27.1844 76.4605 26.5886C76.5341 26.2811 76.5704 25.9657 76.5686 25.6494V24.382Z"
        fill="#E7E6D9"
      />
    </svg>
  );
}

function ToggleIcon() {
  return (
    <span className="toggleIcon">
      <span className="toggleIconBar" />
      <span className="toggleIconBar" />
    </span>
  );
}

type DropdownLink = { label: string; href: string; external?: boolean };
type DropdownColumn = {
  heading: string;
  links: DropdownLink[];
  footer?: DropdownLink;
};

const PRODUCTS_DROPDOWN: DropdownColumn[] = [
  {
    heading: "Supercomputers",
    links: [
      { label: "Superclusters", href: "https://lambda.ai/superclusters" },
      { label: "1-Click Clusters™", href: "https://lambda.ai/1-click-clusters" },
      { label: "Instances", href: "https://lambda.ai/instances" },
    ],
    footer: { label: "Compare", href: "https://lambda.ai/cloud" },
  },
  {
    heading: "For every mission",
    links: [
      { label: "Superintelligence", href: "https://lambda.ai/superintelligence" },
      { label: "Enterprise", href: "https://lambda.ai/enterprise" },
      { label: "Government", href: "https://lambda.ai/government" },
      { label: "Startups and Researchers", href: "https://lambda.ai/instances" },
    ],
  },
  {
    heading: "Foundations",
    links: [
      { label: "AI factories", href: "https://lambda.ai/ai-infrastructure" },
      { label: "Orchestration", href: "https://lambda.ai/orchestration" },
      { label: "Lambda Stack", href: "https://lambda.ai/lambda-stack-deep-learning-software" },
      { label: "Trust and security", href: "https://lambda.ai/trust" },
    ],
  },
  {
    heading: "Docs",
    links: [
      { label: "Customer stories", href: "https://lambda.ai/customer-stories" },
      { label: "Documentation", href: "https://docs.lambda.ai", external: true },
      { label: "Blog", href: "https://lambda.ai/blog" },
      { label: "Research", href: "https://lambda.ai/research" },
    ],
  },
];

const COMPANY_DROPDOWN: DropdownColumn[] = [
  {
    heading: "Inside Lambda",
    links: [
      { label: "About", href: "https://lambda.ai/about" },
      { label: "Careers", href: "https://lambda.ai/careers" },
      { label: "Leadership", href: "https://lambda.ai/leadership" },
      { label: "Investors", href: "https://lambda.ai/investors" },
    ],
  },
  {
    heading: "Resources",
    links: [
      { label: "Research", href: "https://lambda.ai/research" },
      { label: "LLM index", href: "https://lambda.ai/inference-models" },
      { label: "GPU benchmarks", href: "https://lambda.ai/gpu-benchmarks" },
      { label: "Customer stories", href: "https://lambda.ai/customer-stories" },
      { label: "Support", href: "https://lambda.ai/support" },
      { label: "Blog", href: "https://lambda.ai/blog" },
      { label: "Partners", href: "https://lambda.ai/partners" },
    ],
  },
];

const GET_STARTED_DROPDOWN: DropdownColumn[] = [
  {
    heading: "",
    links: [{ label: "Create account", href: "https://lambda.ai/sign-up" }],
    footer: { label: "Talk to our team", href: "https://lambda.ai/talk-to-our-team" },
  },
];

function DesktopDropdown({
  id,
  columns,
}: {
  id: string;
  columns: DropdownColumn[];
}) {
  return (
    <div id={id} className="dropdownWrapper">
      <div className="dropdownContent">
        <ul className="dropdownColumns">
          {columns.map((column) => (
            <li key={column.heading || column.links[0]?.label}>
              {column.heading ? (
                <div className="dropdownColumnHeading">{column.heading}</div>
              ) : null}
              <ul className="dropdownLinks">
                {column.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      aria-label={link.label}
                      tabIndex={0}
                      {...(link.external
                        ? { target: "_blank", rel: "noopener noreferrer" }
                        : {})}
                    >
                      {link.label}
                    </a>
                  </li>
                ))}
              </ul>
              {column.footer ? (
                <div className="dropdownFooter">
                  <a href={column.footer.href} aria-label={column.footer.label} tabIndex={0}>
                    {column.footer.label}
                  </a>
                </div>
              ) : null}
            </li>
          ))}
        </ul>
      </div>
    </div>
  );
}

function MobileDropdown({
  id,
  columns,
  isOpen,
}: {
  id: string;
  columns: DropdownColumn[];
  isOpen: boolean;
}) {
  return (
    <div id={id} className={`mobileDropdown${isOpen ? " active" : ""}`}>
      <div className="mobileDropdownContent">
        {columns.map((column) => (
          <div className="mobileDropdownColumn" key={column.heading || column.links[0]?.label}>
            {column.heading ? (
              <div className="mobileDropdownColumnHeading">{column.heading}</div>
            ) : null}
            <ul className="mobileDropdownLinks">
              {column.links.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    aria-label={link.label}
                    {...(link.external
                      ? { target: "_blank", rel: "noopener noreferrer" }
                      : {})}
                  >
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
            {column.footer ? (
              <div className="mobileDropdownFooter">
                <a href={column.footer.href} aria-label={column.footer.label}>
                  {column.footer.label}
                </a>
              </div>
            ) : null}
          </div>
        ))}
      </div>
    </div>
  );
}

export default function Header() {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openMobileDropdown, setOpenMobileDropdown] = useState<string | null>(null);

  const closeMobileMenu = useCallback(() => {
    setMobileOpen(false);
    setOpenMobileDropdown(null);
  }, []);

  const toggleMobileMenu = useCallback(() => {
    setMobileOpen((open) => {
      if (open) {
        setOpenMobileDropdown(null);
      }
      return !open;
    });
  }, []);

  const toggleMobileDropdown = useCallback((id: string) => {
    setOpenMobileDropdown((current) => (current === id ? null : id));
  }, []);

  useEffect(() => {
    document.body.classList.toggle("mobile-menu-open", mobileOpen);
    return () => {
      document.body.classList.remove("mobile-menu-open");
    };
  }, [mobileOpen]);

  useEffect(() => {
    if (!mobileOpen) return;

    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        closeMobileMenu();
      }
    };

    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [mobileOpen, closeMobileMenu]);

  return (
    <header
      className="siteHeader"
      style={{ ["--total-header-height" as string]: "var(--header-nav-height)" }}
    >
      <div className="container">
        <div className="headerContentWrapper">
          <div className="headerLogoWrapper">
            <a href="/" className="headerLogo" aria-label="Lambda homepage">
              <LambdaLogo />
            </a>
          </div>

          <nav className="mainNav" aria-label="Primary">
            <ul>
              <li>
                <a
                  href="https://lambda.ai/ai-infrastructure"
                  className="directLink"
                  aria-label="AI factories"
                  tabIndex={0}
                >
                  AI factories
                </a>
              </li>
              <li className="hasDropdown">
                <a
                  className="noLink"
                  aria-label="Products"
                  tabIndex={0}
                  aria-haspopup="true"
                  aria-expanded="false"
                  aria-controls="dropdown-products"
                  role="button"
                >
                  Products
                </a>
                <DesktopDropdown id="dropdown-products" columns={PRODUCTS_DROPDOWN} />
              </li>
              <li>
                <a
                  href="https://lambda.ai/pricing"
                  className="directLink"
                  aria-label="Pricing"
                  tabIndex={0}
                >
                  Pricing
                </a>
              </li>
              <li className="hasDropdown">
                <a
                  className="noLink"
                  aria-label="Company"
                  tabIndex={0}
                  aria-haspopup="true"
                  aria-expanded="false"
                  aria-controls="dropdown-company"
                  role="button"
                >
                  Company
                </a>
                <DesktopDropdown id="dropdown-company" columns={COMPANY_DROPDOWN} />
              </li>
            </ul>
          </nav>

          <div className="utilityNav">
            <ul>
              <li>
                <a
                  href="https://lambda.ai/login"
                  className="directLink"
                  aria-label="Log in"
                  tabIndex={0}
                >
                  Log in
                </a>
              </li>
              <li className="hasDropdown">
                <a
                  className="noLink"
                  aria-label="Get started"
                  tabIndex={0}
                  aria-haspopup="true"
                  aria-expanded="false"
                  aria-controls="dropdown-get-started"
                  role="button"
                >
                  Get started
                </a>
                <DesktopDropdown id="dropdown-get-started" columns={GET_STARTED_DROPDOWN} />
              </li>
            </ul>
          </div>

          <button
            type="button"
            className="mobileMenuToggle"
            aria-label="Toggle mobile menu"
            aria-expanded={mobileOpen}
            onClick={toggleMobileMenu}
          >
            <span className={`hamburger${mobileOpen ? " active" : ""}`}>
              <span />
              <span />
              <span />
            </span>
          </button>
        </div>
      </div>

      <div
        className={`mobileMenuOverlay${mobileOpen ? " active" : ""}`}
        aria-hidden={!mobileOpen}
        onClick={closeMobileMenu}
      />

      <nav
        className={`mobileMenu${mobileOpen ? " active" : ""}`}
        aria-label="Mobile navigation"
        aria-hidden={!mobileOpen}
      >
        <div className="mobileMenuContent">
          <div className="mobileNavSection">
            <ul className="mobileNavList">
              <li className="mobileNavItem">
                <a
                  href="https://lambda.ai/ai-infrastructure"
                  className="mobileNavLink"
                  aria-label="AI factories"
                >
                  AI factories
                </a>
              </li>
              <li className="mobileNavItem">
                <button
                  type="button"
                  className={`mobileNavToggle${openMobileDropdown === "products" ? " active" : ""}`}
                  aria-expanded={openMobileDropdown === "products"}
                  aria-controls="mobile-dropdown-products"
                  onClick={() => toggleMobileDropdown("products")}
                >
                  Products
                  <ToggleIcon />
                </button>
                <MobileDropdown
                  id="mobile-dropdown-products"
                  columns={PRODUCTS_DROPDOWN}
                  isOpen={openMobileDropdown === "products"}
                />
              </li>
              <li className="mobileNavItem">
                <a href="https://lambda.ai/pricing" className="mobileNavLink" aria-label="Pricing">
                  Pricing
                </a>
              </li>
              <li className="mobileNavItem">
                <button
                  type="button"
                  className={`mobileNavToggle${openMobileDropdown === "company" ? " active" : ""}`}
                  aria-expanded={openMobileDropdown === "company"}
                  aria-controls="mobile-dropdown-company"
                  onClick={() => toggleMobileDropdown("company")}
                >
                  Company
                  <ToggleIcon />
                </button>
                <MobileDropdown
                  id="mobile-dropdown-company"
                  columns={COMPANY_DROPDOWN}
                  isOpen={openMobileDropdown === "company"}
                />
              </li>
            </ul>
          </div>

          <div className="mobileUtilitySection">
            <ul className="mobileNavList">
              <li className="mobileNavItem">
                <a href="https://lambda.ai/login" className="mobileNavLink" aria-label="Log in">
                  Log in
                </a>
              </li>
              <li className="mobileNavItem">
                <button
                  type="button"
                  className={`mobileNavToggle${openMobileDropdown === "get-started" ? " active" : ""}`}
                  aria-expanded={openMobileDropdown === "get-started"}
                  aria-controls="mobile-dropdown-get-started"
                  onClick={() => toggleMobileDropdown("get-started")}
                >
                  Get started
                  <ToggleIcon />
                </button>
                <MobileDropdown
                  id="mobile-dropdown-get-started"
                  columns={GET_STARTED_DROPDOWN}
                  isOpen={openMobileDropdown === "get-started"}
                />
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </header>
  );
}
